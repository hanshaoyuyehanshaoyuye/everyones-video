...
## ADR-6: SQI 字幕质量引擎 — 翻译后自动修复

### 背景

可乐鸡翅反馈 5 大实际问题：字幕重叠、长间隔滞留、过短闪屏、CPS 过高不可读、间距处理。

### 决策

**翻译后自动跑 SQI（subtitle_quality.py），pipeline.sh / translate_srt.py 默认开启，`--no-sqi` 跳过。**
...
|------|------|------|
| 重叠 | 检测 → 推后 start 时间 | — |
| 过长滞留 | 截断到 MAX_CUE_DURATION | 7s |
| 过短闪屏 | 拉长到 MIN_CUE_DURATION | 0.8s |
| 间距过短 | 合并相邻字幕 | < 0.15s |
| CPS 过高 | 仅警告不阻断 | zh≤8, en≤20 |

### 理由

1. **无额外依赖** — 纯 Python 算法，不调 API
2. **集成透明** — translate_srt.py / pipeline.sh 自动运行，用户无感
3. **独立可用** — `python3 subtitle_quality.py input.srt --fix` 可单独使用
...
| SQI 字幕质量 | Python 纯算法 | 重叠修复/时长截断/CPS 检查（v7.0） |
